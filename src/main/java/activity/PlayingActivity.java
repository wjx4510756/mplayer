...
public class PlayingActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private ImageView mBack, albumImg, pre, next, playBtn, list, mode, share;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        lycShow = StaticUtil.sharedPreferences.getBoolean("lyc", true);

        if (!lycShow) {
            lyc.setVisibility(View.GONE);
            setAlbumImg();
        } else
            albumImg.setVisibility(View.GONE);
    ...
    private void init() {
        ...
        albumImg = (ImageView) findViewById(R.id.album_img);
        ...
        seekBar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            ...
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                if (fromUser && StaticUtil.currentPosition != -1) {
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.share:
                Toast.makeText(PlayingActivity.this, "分享", Toast.LENGTH_SHORT).show();
                break;
            default:
                break;
    ...
    private void setAlbumImg() {
        if (StaticUtil.currentPosition != -1) {
            String url = currentSongList.get(StaticUtil.currentPosition).getFileUrl();
            String path = url.substring(0, url.length() - 3) + "jpg";
            Bitmap bitmap = MusicUtil.decodeSampledBitmapFromFile(path, 300, 300);
            if (bitmap != null) {
                albumImg.setImageBitmap(bitmap);
            } else
                albumImg.setImageResource(R.drawable.placeholder_disk_300);
        }
    }
}
